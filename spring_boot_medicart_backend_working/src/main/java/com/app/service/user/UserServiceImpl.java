...
//			if (matches(oldPassword, authCust.getPassword())) {
//				authCust.setPassword(newPassword);
...
///userEntity.setPassword(encoder.encode(user.getPassword()));
	...
	public UserDto authenticateUser(AuthRequest request) {
	    // Invoke the repository method to find the user by email and password
	    User emp = userRepositary.findByEmailAndPassword(request.getEmail(), request.getPassword())
	            .orElseThrow(() -> new ResourceNotFoundException("Invalid email or password!!!!"));

	    // Map the User entity to UserDto
	    return mapper.map(emp, UserDto.class);
	}
